...
def index():
    ...
    ## Check if session variable corresponds to a previous experiment; if so, clear
    if 'complete' in session and session['code_success'] != cfg['PROLIFIC'].get('CODE_SUCCESS') and session['code_success'] != cfg['PROLIFIC'].get('CODE_REJECT'):

        ## Update metadata.
        session['WARNING'] = "Visited with a previous cookie intact."
